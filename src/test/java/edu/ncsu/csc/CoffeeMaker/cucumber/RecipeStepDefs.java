...
public class RecipeStepDefs {
    ...
    public RecipeStepDefs () {
        this.recipeData = new SharedRecipeData();
    }
    ...
    public void existingRecipes ( final int numRecipes ) throws Exception {
        ...
        final int total = recipeService.findAll().size();
        if ( total != 0 ) {
            recipeService.deleteAll();
            Assert.assertEquals( 0, recipeService.findAll().size() );
        }
    ...
    public void invalidDeleteByName ( final String name ) {
        recipeData.currentRecipeList = (List<Recipe>) recipeService.findAll();
    ...
    public void recipeEditedSuccessfully () {
        Assert.assertEquals( "Recipe was not edited correctly", recipeData.currentRecipe,
                recipeService.findByName( recipeData.currentRecipe.getName() ) );
    }
    ...
    public void recipeEditedUnsuccessfully () {
        ...
        final Recipe other = recipeService.findByName( recipeData.currentRecipe.getName() );
    ...
    public void recipeDeletedByIndex () {
        final List<Recipe> recipes = (List<Recipe>) recipeService.findAll();
    ...
    public void recipeNotAdded () {
        ...
        final boolean recipeInList = recipeService.findAll().contains( recipeData.currentRecipe );
    ...
    public void secondRecipeNotAdded () {
        ...
        for ( final Recipe r : (List<Recipe>) recipeService.findAll() ) {
